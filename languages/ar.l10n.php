<?php
return [
    'language' => 'ar',
    'language-team' => 'العربية',
    'messages' => [
        '#1 on Google for "أكل قطط في دمشق"' => 'زيارات ومبيعات فعلية من الظهور الممتاز على غوغل (جرب ابحث "أكل قطط في دمشق")',
        'Abdullatif Al-Mayhob' => 'عبد اللطيف الميهوب',
        'Arabic' => 'العربية',
        'Available in:' => 'متوفر في:',
        'Yes, both as part-time job, and as a frelancer (details below)' => 'نعم, متوفر للعمل بارت تايم او كـ فريلانسر (التفصيل ادناه)',
        'Completed Projects​' => 'المشاريع المكتملة',
        'Contact Me' => 'تواصل معي',
        'Custom Themes and Custom Plugins ..' => 'قوالب و اضافات خاصة حسب الطلب ...',
        'Check for youself' => 'اضغط هنا وجرب بنفسك',
        'Download CV' => 'تحميل الـCV',
        'Do you like this website ?' => 'حبيت الموقع ؟ بدك متله ؟',
        'Senior Wordpress PHP Developer' => 'مطور ووردبريس محترف',
        'E-Mail:' => 'البريد الإلكتروني',
        'English' => 'الإنكليزية',
        'Flexibility, Creativity' => 'المرونة و الإبداع',
        'French' => 'الفرنسية',
        'Fast even on 3G networks' => 'سريع حتى على شبكات الـ3G',
        'Fully custom theme and plugins.' => 'كود ثيم خاص.',
        'Go back to Abdoo' => 'العودة إلى كوكب الأرض',
        'Happy Customer' => 'زبون راض',
        'hbrarabic.com' => 'مجلة هارفرد بزنس رفيو بالعربية',
        'Hey There, I\'m Abdoo' => 'مرحباً, انا عبدو',
        'Highly customized store with exceptional UX.' => 'متجر تخصصي عالي الاداء, يستهدف السوق الدمشقي.',
        'Honors and Awards' => 'الجوائز و الشهادات',
        'Is Abdoo Availabe for Work ?' => 'هل عبدو متوفر للعمل ؟',
        'I Develop' => 'انا اطور', 'I like talking, meeting new people, making new friends. Let\'s work on a new project. Let\'s build a startup together. Let\'s leave the the earth and conquer the space. Let\'s eliminate all the boundaries to our dreams. It would be a pleasure to know you. Contact Me !' => 'أحب التحدث والتعرف على أشخاص جدد وتكوين صداقات جديدة. لنعمل على مشروع جديد. دعنا نبني شركة ناشئة معًا. فلنترك الأرض ونحتل الفضاء. و لنتخلص من كل الحدود لأحلامنا.
سيكون من دواعي سروري أن أتعرف عليك. تواصل معي!                    ',
        'Languages' => 'اللغات',
        'Looking for a Senior <br><span class="clr-acc">Wordpress</span> Expert ?' => '
        عم تدور على حدا
<span class="clr-acc">كفؤ</span> و
<span class="clr-acc">خبير</span>
        بالـووردبريس لشغلك الخاص ؟
        ',
        'Koki.sy e-com for cats in Damascus' => 'متجر كوكي لمستلزمات القطط في دمشق',
        'Leadership' => 'القيادة',
        'Let\'s Make Wonders Together!' => 'فلنقم بالمستحيل معاً !',
        'Let\'s Talk About Everything !' => 'فلنتحدث عن كل شيء!',
        'Live Websites I Worked On' => 'مواقع عملت فيها',
        'Load More' => 'تحميل المزيد',
        'Lost in space. No where to go <br>Don\'t scream, Nobody can hear you' => 'ضائع في الفضاء, لا مكان للذهاب إليه <br> لا تصرخ, لن يسمعك أحد',
        '- Managing and Developing complex enterprise wordpress sites. <br>- Buliding Advanced Customized Themes and Plugins.' => '- بناء مواقع ويب و متاجر الكترونية عالية التخصص و الاداء تحقق ربح فعلي.  ',
        'Optimizing your site to last byte and Setting up the latest Tech and Speed Hacks while maintaining Stability and Simplicity. Boosting page load time and Site Search engines Ranking.' => 'تحسين اداء موقعك حتى آخر بايت, إعداد أحدث التقنيات والحيل في زيادة سرعة الموقع وتحسين السيو الخاص به مع الحفاظ على الاستقرار والبساطة. تعزيز وقت تحميل الصفحة ورفع تصنيف محركات البحث لموقعك.',
        "Let's make you one 🙋🏻‍♂️" => 'خلينا ندردش 🙋🏻‍♂️',
        'Phone' => 'جوال',
        'Problem Solving' => 'حل المشاكل خارج الصندوق',
        'Portfolio' => 'معرض الأعمال',
        'Professional Responsive sites built with the newest and highest standards ensuring the best SEO results and excellent performance.' => 'بناء مواقع احترافية متوافقة مع جميع احجام الشاشات بأحدث وأعلى المعايير لضمان أفضل نتائج السيو والأداء ممتاز.',
        'Request your own design and functionality to server your business needs. Ensuring high reliability, best performance and latest code standards and best practices.' => 'اطلب تصميمك و الميزات الخاصة بك لخدمة احتياجات عملك. ضمان الاعتمادية العالية وأفضل أداء وأحدث معايير جودة الكود.',
        'Questions about Abdoo' => 'شو بتحب تعرف عني',
        'Residence' => 'اين انا ناو',
        'Vibes lately?' => 'شو عم تسمع هالأيام؟',
        'Services' => 'الخدمات',
        'Mazza, Damascus' => 'دمشق, المزة',
        'Technical SEO And Performance Optimization' => 'تحسين محركات البحث و الاداء',
        'Telegram:' => 'تيليغرام:',
        'Top SEO + blazing fast performance.' => 'سيو ممتاز, و اداء عالي.',
        'Top Skills' => 'المهارات المميزة',
        'View much more on my Linked-In >>' => 'شاهد المزيد من التوصيات على اللينكد الخاص بي',
        'Wordpress' => 'ووردبريس',
        'English and Arabic fluently. French Learner.' => 'الإنكليزية و العربية طبعاً. <br> هاوي في اللغة الفرنسية',
        'What Languages he speaks ?' => 'شو اللغات يلي بتحكيها ؟',
        'What does Abdoo bring to your team ?' => 'شو القيمة يلي بضيفها عبدو على فريقك ؟',
        'What are the last 2 projects you worked on ?' => 'شو آخر مشروعين اشتغلت عليهم ؟',
        'WordPress Development' => 'تطوير مواقع ووردبريس',
        'Words About Abdoo' => 'قالوا عن عبداللطيف',
        "What is Abdoo's main expertise ?" => 'وين خبرتك, مهاراتك واهتماماتك ؟',
        'WP Themes & Plugins' => 'قوالب و إضافات ووردبريس',
        'Years of Experience' => 'سنوات الخبرة',
    ]
];
